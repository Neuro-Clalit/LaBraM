...
def main(args):
    ...
    dataset_train_list, train_ch_names_list = utils.build_pretraining_dataset(
        datasets_train, time_window, stride=200,
    )

    datasets_val = [["path/to/datasets_val"]]
    if args.disable_eval:
        dataset_val_list = None
        val_ch_names_list = None
    else:
    ...
    if dataset_val_list is not None:
        sampler_eval_list = runner_common.build_distributed_eval_sampler_list(
            dataset_val_list, num_tasks, global_rank, args.dist_eval,
        )
    else:
    ...
    if args.eval:
        evaluate(data_loader_val_list, model, device, log_writer, 0, args=args)
        return

    if args.calculate_codebook_usage:
        calculate_codebook_usage(data_loader_val_list, model, device, log_writer, 0, args=args)
        return
