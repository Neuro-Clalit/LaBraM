...
- `labram/losses/` — Configurable training losses: `config.py` (`LossConfig` dataclass of weights/options, defaults reproduce original behavior), `spectral.py` (`SpectralReconstructionLoss` — FFT amplitude/phase recon), `vqnsp.py` (`get_vqnsp_losses` weighted aggregator), `classification.py` (`build_classification_criterion` — BCE/CE/LabelSmoothing selection)
