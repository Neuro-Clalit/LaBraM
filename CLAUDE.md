...
# Brain-age (TUAB_AGE) fine-tune on SageMaker. The age label lives in an
# age_metadata.json sidecar, not the window pickles, so upload both sidecars into
# the same S3 prefix as the windows first; the submitter fails fast if the
# metadata one is missing. Add cross_validation.enabled=true for one job per fold.
