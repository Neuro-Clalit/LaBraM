...
from typing import TypeVar, List, Literal, Optional, \
...
def correct_type(var: Any, type_hint: Type) -> bool:
    ...
    Handles List, Optional, Union, Literal, Sequence, and Dict from typing module.
    """
    ...
    # Literal["a", "b", ...] — var must be one of the literal values
    if get_origin(type_hint) is Literal:
        return var in get_args(type_hint)
    ...
    if origin_type is Union or isinstance(origin_type, TypeVar):
        ...
        if not isinstance(var, (list, tuple, np.ndarray)):
            return False
        if not args:  # bare List / Sequence with no element type — accept any sequence
            return True
        element_type = args[0]
